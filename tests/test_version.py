...
class VersionTest(unittest.TestCase):
    def test_version_cases(self):
        self.assertEqual(str(v0 := Version(11, 45, 14, PreRelease())), '11.45.14')
        self.assertEqual(str(va1 := Version(11, 45, 14, PreRelease().alpha(1))), '11.45.14-alpha.1')
        self.assertEqual(str(vb2 := Version(11, 45, 14, PreRelease().beta(2))), '11.45.14-beta.2')
        self.assertEqual(str(vrc3 := Version(11, 45, 14, PreRelease().rc(3))), '11.45.14-rc.3')
    ...
    def test_error(self):
        self.assertRaises(InvalidCall, PreRelease(), 'nothing', 'no', 0)
        self.assertRaises(LockedVersion, PreRelease().alpha(1).alpha, 1)
        self.assertRaises(LockedVersion, PreRelease().beta(2).beta, 2)
        self.assertRaises(LockedVersion, PreRelease().rc(3).rc, 3)
